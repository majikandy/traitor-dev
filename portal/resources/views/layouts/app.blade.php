    ...
    <div class="flex h-full">
        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-30 w-64 bg-sidebar-bg transform -translate-x-full transition-transform duration-200 ease-in-out lg:translate-x-0 lg:static lg:inset-auto flex flex-col">
            ...
            <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
                ...
                @if(auth()->user()->is_admin)
                    ...
                    <a href="{{ route('admin.cpanel') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium {{ request()->is('admin/cpanel') ? 'bg-sidebar-active text-white' : 'text-sidebar-text hover:bg-sidebar-hover' }} transition">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M5.25 14.25h13.5m-13.5 0a3 3 0 0 1-3-3m3 3a3 3 0 1 0 6 0m-6 0H3m16.5 0a3 3 0 0 0 3-3m-3 3a3 3 0 1 1-6 0m6 0h1.5m-7.5 0V5.625A2.25 2.25 0 0 1 12 3.375v0a2.25 2.25 0 0 1 2.25 2.25V14.25" /></svg>
                        cPanel
                    </a>
        ...
        <div class="flex-1 flex flex-col min-h-screen lg:min-h-0 overflow-auto">
            ...
            <main class="flex-1 p-4 sm:p-6 lg:p-8">
                ...
                @endif
                @if(session('error'))
                    @php $errorMeta = session('error_meta', []); @endphp
                    <div class="mb-6 flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 flash flash-persist"
                         data-action="{{ $errorMeta['action'] ?? '' }}"
                         data-input="{{ isset($errorMeta['input']) ? json_encode($errorMeta['input']) : '' }}">
                        <svg class="h-5 w-5 text-red-500 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 3.75h.008v.008H12v-.008Z" /></svg>
                        <span class="flex-1">{{ session('error') }}</span>
                        <button onclick="(function(btn){
                            var el=btn.closest('.flash-persist');
                            var lines=['Page: '+window.location.pathname,'Action: '+(el.dataset.action||'unknown'),'Error: '+el.querySelector('span').textContent.trim()];
                            var inp=el.dataset.input;
                            if(inp){try{var obj=JSON.parse(inp);var pairs=Object.entries(obj).map(function(e){return e[0]+': '+e[1]});if(pairs.length)lines.push('Input: '+pairs.join(', '));}catch(e){}}
                            navigator.clipboard.writeText(lines.join('\n')).then(function(){btn.textContent='Copied!';setTimeout(function(){btn.textContent='Copy'},1500)});
                        })(this)" class="text-red-500 hover:text-red-700 transition shrink-0 text-xs font-medium border border-red-300 rounded px-2 py-0.5">Copy</button>
                        <button onclick="this.closest('.flash-persist').remove()" class="text-red-400 hover:text-red-600 transition shrink-0 leading-none text-base">&times;</button>
                    </div>
                @endif
