    ...
    @if($site->github_repo)
        ...
        <div class="ml-12 space-y-4">
            ...
            <form method="POST" action="{{ route('github.disconnect', $site) }}" class="pt-1">
                @csrf
                @method('DELETE')
                <button type="submit" class="text-xs text-red-500 hover:text-red-700 transition" onclick="return confirm('Disconnect {{ $site->github_repo }}?')">Disconnect GitHub</button>
            </form>
        </div>
    @else
