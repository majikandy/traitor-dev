...
<div class="rounded-xl border border-gray-200 bg-white shadow-sm mb-6">
    <div class="flex w-full items-center gap-3 px-6 py-4">
        ...
        @if($site->github_repo)
        <form method="POST" action="{{ route('github.auto-deploy', $site) }}" class="flex items-center gap-2 flex-shrink-0 ml-1" onclick="event.stopPropagation()">
            @csrf
            <span class="text-xs text-gray-400 hidden sm:block">Auto&nbsp;current</span>
            <button type="submit" title="{{ $site->github_auto_deploy ? 'Auto make current: on — click to disable' : 'Auto make current: off — click to enable' }}"
                class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 {{ $site->github_auto_deploy ? 'bg-brand-600' : 'bg-gray-200' }}">
